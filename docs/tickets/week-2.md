...
- Status: `merged, PR #25` (deploy พักไว้ — T29/T30 ยังไม่เริ่ม) — Dockerfile (Go API, alpine 74MB, non-root) + web/Dockerfile (Next export→caddy:2) + deploy/{docker-compose.prod.yml, Caddyfile} + .dockerignore + .env.example. verify ผ่าน Caddy จริง (healthz/curriculum/lesson 200), least-privilege: caddy/mysql ไม่ได้ secret เกินจำเป็น. code-reviewer APPROVE
