...
type fakeRepository struct {
	...
	// question texts and their kind, keyed case-folded (canonical question
	// as the value) so a case-variant question resolves the same way the
	// real case-insensitive JOIN in infra would (R1). kind is resolved from
	// the seeded row here, exactly as Repository.RecordAttempt resolves it
	// from rc.type — never taken from any caller-supplied value, since kind
	// is no longer part of Service.RecordAttempt's signature at all (R1).
	recallChecks map[conceptKey]map[string]recallCheckSeed

	recordAttemptErr   error
	recordAttemptCalls []recordAttemptCall
}

type recallCheckSeed struct {
	question string
	kind     string
}
...
func (f *fakeRepository) withRecallCheck(topic, concept, question, kind string) *fakeRepository {
	f.lessons[conceptKey{topic, concept}] = true
	if f.recallChecks == nil {
		f.recallChecks = map[conceptKey]map[string]recallCheckSeed{}
	}
	k := conceptKey{topic, concept}
	if f.recallChecks[k] == nil {
		f.recallChecks[k] = map[string]recallCheckSeed{}
	}
	f.recallChecks[k][foldQuestion(question)] = recallCheckSeed{question: question, kind: kind}
	return f
}
...
// (as-seeded) question text and kind, never anything a caller of
// Service.RecordAttempt could supply — the same resolve-then-key contract
// the real repository implements (R1).
func (f *fakeRepository) RecordAttempt(_ context.Context, topic, concept, question string, build func(domain.CanonicalQuestion, domain.CheckKind) (domain.RecallAttempt, error)) (AttemptRecord, error) {
	...
	seed, ok := f.recallChecks[k][foldQuestion(question)]
	if !ok {
		return AttemptRecord{}, ErrCheckNotInLesson
	}

	canonicalQuestion, err := domain.NewCanonicalQuestion(seed.question)
	if err != nil {
		return AttemptRecord{}, err
	}
	kind, err := domain.NewCheckKind(seed.kind)
	if err != nil {
		return AttemptRecord{}, err
	}
...
func TestServiceRecordAttempt_LessonNotFound(t *testing.T) {
	...
	_, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "What is a B-tree?", "unsure", "correct", nil)
	if !errors.Is(err, ErrLessonNotFound) {
...
func TestServiceRecordAttempt_CheckNotInLesson(t *testing.T) {
	repo := newFakeRepository().withRecallCheck("ddia", "b-trees", "What is a B-tree?", "short_answer")
	svc := NewService(repo)

	_, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "A different question entirely", "unsure", "correct", nil)
...
func TestServiceRecordAttempt_InvalidSlug(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := newFakeRepository().withRecallCheck(tt.topic, tt.concept, "q", "short_answer")
			svc := NewService(repo)

			_, err := svc.RecordAttempt(context.Background(), tt.topic, tt.concept, "q", "unsure", "correct", nil)
			if !errors.Is(err, ErrInvalidSlug) {
...
func TestServiceRecordAttempt_InvalidFields(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid confidence", confidence: "certain", outcome: "correct"},
		{name: "empty confidence", confidence: "", outcome: "correct"},
		{name: "invalid outcome", confidence: "unsure", outcome: "partial"},
		{name: "empty outcome", confidence: "unsure", outcome: ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := newFakeRepository().withRecallCheck("ddia", "b-trees", "q", "short_answer")
			svc := NewService(repo)

			_, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "q", tt.confidence, tt.outcome, nil)
...
// RecallAttempt's own invariant surfacing as ErrInvalidAttempt, with kind
// resolved from the repository's seeded recall check (R1) rather than any
// client-supplied value. The repository IS invoked (it must resolve the
// canonical question and kind before the CheckKey — and therefore the
// RecallAttempt — can even be built), but build() itself must reject before
// anything is ever inserted.
func TestServiceRecordAttempt_SelectedOptionOnShortAnswerRejected(t *testing.T) {
	repo := newFakeRepository().withRecallCheck("ddia", "b-trees", "q", "short_answer")
	svc := NewService(repo)

	selected := "Option B"
	_, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "q", "unsure", "correct", &selected)
	if !errors.Is(err, ErrInvalidAttempt) {
		t.Fatalf("RecordAttempt() error = %v, want it to wrap ErrInvalidAttempt", err)
	}
...
// exactly as sent, not a hardcoded stand-in — table-driven over all three
// Confidence values (R5) so the assertion isn't tied to a single value
// "confident" happening to also be, say, the zero value of some type.
func TestServiceRecordAttempt_EchoesSubmittedValues(t *testing.T) {
	tests := []struct {
		confidence string
		outcome    string
	}{
		{confidence: "guessed", outcome: "correct"},
		{confidence: "unsure", outcome: "incorrect"},
		{confidence: "confident", outcome: "incorrect"},
	}
	for _, tt := range tests {
		t.Run(tt.confidence, func(t *testing.T) {
			repo := newFakeRepository().withRecallCheck("ddia", "b-trees", "Which option is correct?", "mcq")
			svc := NewService(repo)

			selected := "Option B"
			entry, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "Which option is correct?", tt.confidence, tt.outcome, &selected)
			if err != nil {
			...
			if entry.Confidence.String() != tt.confidence {
				t.Errorf("Confidence = %q, want %q", entry.Confidence.String(), tt.confidence)
			}
			if entry.Outcome.String() != tt.outcome {
				t.Errorf("Outcome = %q, want %q", entry.Outcome.String(), tt.outcome)
			}
			...
			if call.attempt.Confidence().String() != tt.confidence || call.attempt.Outcome().String() != tt.outcome {
				t.Errorf("repository received attempt %+v, want the submitted confidence=%q/outcome=%q unchanged", call.attempt, tt.confidence, tt.outcome)
			}
		})
	}
}
...
func TestServiceRecordAttempt_CheckKeyDerivedFromCanonicalQuestion(t *testing.T) {
	...
	entry, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "what is a b-tree?", "unsure", "correct", nil)
	if err != nil {
		t.Fatalf("RecordAttempt() unexpected error: %v", err)
	}
	...
	wantKey, err := domain.NewCheckKey("ddia", conceptRef, mustCanonicalQuestion(t, "What is a B-tree?"))
	if err != nil {
	...
	rawKey, err := domain.NewCheckKey("ddia", conceptRef, mustCanonicalQuestion(t, "what is a b-tree?"))
	if err != nil {
		t.Fatalf("NewCheckKey() failed: %v", err)
	}
...
func TestServiceRecordAttempt_EmptyQuestionRejected(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", tt.question, "unsure", "correct", nil)
			if !errors.Is(err, ErrInvalidAttempt) {
...
func TestServiceRecordAttempt_EmptySelectedOptionTreatedAsAbsent(t *testing.T) {
	repo := newFakeRepository().withRecallCheck("ddia", "b-trees", "q", "mcq")
	svc := NewService(repo)

	empty := "   "
	entry, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "q", "unsure", "correct", &empty)
	if err != nil {
		t.Fatalf("RecordAttempt() unexpected error: %v", err)
	}
...
func TestServiceRecordAttempt_RepoErrorPropagates(t *testing.T) {
	...
	_, err := svc.RecordAttempt(context.Background(), "ddia", "b-trees", "q", "unsure", "correct", nil)
	if !errors.Is(err, repoErr) {
